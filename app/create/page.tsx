...
const formSchema = z.object({
  ...
  maxSupply: z.string().min(1, "Max supply is required").refine((val) => !isNaN(Number(val)) && Number(val) > 0, {
    message: "Max supply must be a positive number",
  }),
  mintPrice: z.string().min(1, "Mint price is required").refine((val) => !isNaN(Number(val)) && Number(val) >= 0, {
  ...
  maxMintsPerWallet: z.string().min(1, "Max mints per wallet is required").refine((val) => !isNaN(Number(val)) && Number(val) > 0, {
    message: "Max mints per wallet must be a positive number",
  }),
  mintStartTime: z.string().optional()
    .refine((val) => {
  ...
  mintEndTime: z.string().optional(),
    ...
    if (data.whitelistOnly && (!data.initialWhitelist || data.initialWhitelist.trim() === '')) {
      return false;
    }
    return true;
  }, {
    message: "Whitelist addresses are required when whitelist only is enabled",
    ...
    message: "Invalid Ethereum addresses format. Use comma-separated addresses",
    path: ["initialWhitelist"],
  }).refine((data) => {
...
export default function CreateCollection() {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const mintStartTime = values.mintStartTime ? Math.floor(new Date(values.mintStartTime).getTime() / 1000) : 0
    const mintEndTime = values.mintEndTime ? Math.floor(new Date(values.mintEndTime).getTime() / 1000) : 0
  ...
  return (
    <div className="mx-auto container max-w-5xl min-h-screen p-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">Deploy your own NFT collection contract</p>
      </div>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Textarea placeholder="Tell us about your NFT collection" {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
      ...
      <Dialog open={showSuccessDialog} onOpenChange={setShowSuccessDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Your NFT collection has been created. You can view the details below.
            </DialogDescription>
          </DialogHeader>
